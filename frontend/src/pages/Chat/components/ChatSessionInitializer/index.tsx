 ...
 *  - majo:sidebar-select-session → switch to the given sessionId
 *  - majo:sidebar-new-chat       → create a new session
 */
const ChatSessionInitializer: React.FC = () => {
  ...
  useEffect(() => {
          ...
          .finally(() => {
            if (!controller.signal.aborted) {
              ...
              window.dispatchEvent(
                new CustomEvent("majo:sidebar-switch-done"),
              );
            }
          });
      }
    ...
    window.addEventListener(
      "majo:sidebar-select-session",
      handleSelectSession,
    );
    window.addEventListener("majo:sidebar-new-chat", handleNewChat);
    ...
    const pendingNewChat = sessionStorage.getItem("majo_pending_new_chat");
    if (pendingNewChat) {
      sessionStorage.removeItem("majo_pending_new_chat");
    ...
    return () => {
      ...
      window.removeEventListener("majo:sidebar-new-chat", handleNewChat);
    };
