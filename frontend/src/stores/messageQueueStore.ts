...
export const STORAGE_PREFIX = "majo:message-queue:";
...
function getChannel(): BroadcastChannel | null {
  ...
  try {
    _channel = new BroadcastChannel("majo:queue");
  } catch {
    _channel = null;
  }
  return _channel;
...
export const useMessageQueueStore = create<MessageQueueStore>((set, get) => ({
  ...
  enqueue: (sessionId: string, input: QueueItemInput) => {
    ...
    try {
      const agentStorage =
        sessionStorage.getItem("majo-agent-storage") ||
        localStorage.getItem("majo-agent-storage");
      if (agentStorage) {
