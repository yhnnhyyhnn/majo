...
/**
 * Protected built-in system prompt content. Faithful port of QwenPaw
 * {@code runtime/protected_prompt.py} (#7526): this contract is part of
 * every agent's system prompt and represents base platform authority — it
 * is not user-editable and must not be treated as overridable by workspace
 * prompt files or untrusted content.
 */
public final class ProtectedPrompt {

    /** The protected execution and authorization contract. */
    public static final String PROTECTED_EXECUTION_CONTRACT = """
    ...
    private ProtectedPrompt() {}

    /**
     * Prepend the protected contract to a base system prompt. The contract
     * comes first so it carries base-platform authority.
     */
    public static String withContract(String sysPrompt) {
        String base = sysPrompt == null ? "" : sysPrompt;
        if (base.contains("# Protected execution contract")) {
